...
from logs.statistics.processing import Log_stats, Ip_stats, Stat_struct
...
from logs.structures.constants import MONTHS, DAYS
...
def print_stats(log_stats: Log_stats,
                output: TextIO,
                geoloc_sample_size: int,
                selected: bool = True,
                year: int = None,
                geoloc_db:Optional[str]= None,
                display_overview_imgs: bool = False) -> None:
    ...
    if log_stats.err_msg:
        timer = Ez_timer("making charts of bots and human users")
    
    if display_overview_imgs:
        display_overview_imgs(log_stats,html)

    print_bots(log_stats,html, selected)
    print_users(log_stats,html, selected)

    if log_stats.err_msg:
        timer.finish()

    print_countries_stats(log_stats, html, geoloc_sample_size, selected)

    print(html.html(), file=output)


def display_overview_imgs(log_stats: Log_stats, html: Html_maker):
...
def test_geolocation(log_stats,
                        output: TextIO,
    ...
    html: Html_maker = Html_maker()
    _test_geolocation(log_stats,html,
                            geoloc_sample_size,
                            selected=selected,
...
def print_bots(log_stats, html: Html_maker, selected=True) -> None:
    ...
    print_most_frequent(html,req_sorted_stats, sess_sorted_stats,
                        True, selected)
...
def print_users(log_stats: Log_stats, html: Html_maker, selected=True):
...
def print_overview(html: Html_maker,
                    header_str: str,
...
def sort_stats(log_stats: Log_stats,
                bot: bool)\
...
def print_most_frequent(html: Html_maker,
    ...
    html.append(make_table(f"Most frequent {group_name} by number of sessions",
                            header,
                            get_most_frequent_table_data(sess_sorted_stats, 20, host_name=host_name, bots=bots),
                            None,
                            ["selectable", selected, uniq_classes[0]],
                            attribs_for_most_freq_table_iter(sess_sorted_stats, 20)
                            ))

    html.append(make_table(f"Most frequent {group_name} by number of requests",
                            header,
                            get_most_frequent_table_data(req_sorted_stats, 20, host_name=host_name, bots=bots),
                            None,
                            ["selectable", selected, uniq_classes[1]],
                            attribs_for_most_freq_table_iter(req_sorted_stats, 20)
                            ))

    html.append("</div>")


def get_most_frequent_table_data(sorted_data: List[Ip_stats], n: int,
                                 bots: bool,  host_name=True)\
        -> List[List]:
    """From decendingly sorted list of Ip_stats 
    return list of `n` rows in most frequent tale"""
    n = min(n, len(sorted_data))
    rows = []

    for i, ip_stat in enumerate(sort_stats[:n]):
...
def attribs_for_most_freq_table_iter(data: List[Ip_stats], n: int)\
        -> List[List[str]]:
    """Returs attributes for `td` elements for most frequent table table"""
    n = min(n, len(data))

    return list(["", "", f"title='{data[i].host_name}'", "", "", ""]
                for i in range(n))
...
def print_day_distribution(log_stats: Log_stats, html: Html_maker, bots, selected=""):
    ...
    table_body = list([f"{i}:00 - {i}:59", str(data.day_req_distrib[i]), str(data.day_sess_distrib[i])] 
                      for i in range(25))
    table_body.append(["Sum", str(sum(data.day_req_distrib)), str(sum(data.day_sess_distrib))])
...
def print_week_distribution(log_stats: Log_stats, html: Html_maker, bots, selected=True):
    group_name = "bots" if bots else "users"
    ...
    data = log_stats.bots if bots else log_stats.people
    table_body = list([DAYS[i], str(data.week_req_distrib[i]), str(data.week_sess_distrib[i])]
                      for i in range(7))
    table_body.append(["Sum", str(sum(data.week_req_distrib)), str(sum(data.week_sess_distrib))])
...
def print_month_distributions(log_stats,
                                html: Html_maker,
                                bots: bool,
                                selected: str = ""):
    group_name = "bots" if bots else "users"  
...
def print_distribution_graph(html: Html_maker,
                                data: List[float],
                                xlabel: str,
                                ylabel: str,
                                x_ticks_labels: List[float],
                                group_name: str,
                                left_margin: bool = False) -> None:
...
def get_geolocations_from_sample(sample: List[Ip_stats]) -> List[Tuple[str,float]]:
    """
    Returns
    -------
    ...
    return sorted(map(lambda x: (x[0], 100*x[1]/weights_sum), geoloc_weights.items()),
                  key=lambda x: x[1],
                  reverse=True)


def _test_geolocation(log_stats,
                        html: Html_maker,
                        geoloc_sample_size: int,
                        repetitions: int = 5,
                        selected: bool = False):
    ...
    # geolocation
    timer = Ez_timer("geolocations", verbose=log_stats.err_msg)

    geoloc_stats = []
    for i, sample in enumerate(samples):
        timer2 = Ez_timer(f"geolocaion {i+1}", verbose=False)
        geoloc_stats.append(get_geolocations_from_sample(sample))
        timer2.finish(log_stats.err_msg)

    timer.finish(log_stats.err_msg)
...
def print_countries_stats(log_stats,
                            html: Html_maker,
                            sample_size: int,
                            selected: bool = False):
    ...
    sample: List[Ip_stats] = list(filter(lambda ip_stat: ip_stat.sessions_num <= 50,
                                         log_stats.people.stats.values()))
    ...
    estimated_locations = get_geolocations_from_sample(sample)
    ...
    table_body = list([i+1, country, round(value, 2)]
...
def print_geolocations_graph(html: Html_maker,
...
def _print_h_bar_graph(html: Html_maker, xs, ys, xlabel, ylabel,
...
def make_histogram(log_stats: Log_stats, file_name:str, selected:str = ""):
    ...
    with open(f"{os.path.dirname(__file__)}/hist.js", "r") as f: #TODO: think of better way to get hist.js
        js = f.read()
    ...
    for year in sorted(log_stats.year_stats.keys()):
        log_stats._switch_years(year)
        print_histograms_for_stats(stats=log_stats.people.stats.values(),
                                   html=html,
                                   title=f"Year {year}",
                                   selected=selected)
...
def print_histograms_for_stats(stats: List[Ip_stats], html:Html_maker, title: str, selected:str = ""):
    session_data = []
    request_data = []
    ...
    _print_histogram_graphs(html=html,
                                data=session_data,
                                bins=300,
    ...
    print_splitted_vals_table(bins, html=html, delims=[2, 5, 10, 50, 100, 1000], 
                                data_name="sessions", selected=selected)
    html.append("</div>")
    ...
    print_splitted_vals_table(bins, html=html, delims=[2, 5, 10, 50, 100, 1000, 10000], 
                                data_name="requests", selected=selected)
    html.append("</div>")
...
def _print_histogram_graphs(html: Html_maker,
                            data: List[int],
                            bins: int,
...
def get_splited_data(data: List[int], 
                        delims: List[int]) -> List[List[int]]:
...
def print_splitted_vals_table(splited_data: List[List[int]], 
                    html: Html_maker, 
                    delims: List[int],
    ...
    for i in range(len(delims)):
        table_body.append([
            ...
            round(100 * len(splited_data[i]) / tot_len, 1) #unique ip addresess in %
        ])
        prev_delim = delims[i]
    ...
    table_body.append([
        f"Above {delims[-1]}",
        data_sums[-1],
        round(100 * data_sums[-1] / tot_sum, 1), len(splited_data[-1]),
    ...
    html.append(make_table(f"{data_name.capitalize()} splitted into bins",
                            ["Frequency [from, to]", f"Sum of {data_name}", "Sum [%]", "Unique IPs", "Unique IPs [%]"],
                            table_body))
    html.append("</div>")
